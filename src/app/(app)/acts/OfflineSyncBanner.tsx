...
import { Cloud, WifiOff, X } from 'react-feather';
...
export default function OfflineSyncBanner() {
  ...
  useEffect(() => {
    // Check storage on mount and when going online
    ...
    startTransition(async () => {
      ...
      for (const act of offlineActs) {
        try {
            ...
            if (result?.errors || (result?.message && result.message !== 'Success')) {
      ...
      if (errors.length > 0) {
        alert(`Не вдалося синхронізувати актів: ${errors.length}. Спробуйте пізніше.`);
      }
    });
  };
